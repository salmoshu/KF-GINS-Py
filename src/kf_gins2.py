...
# def timeAlign(imu_data, gnss_data, starttime):
#     imu_cur = IMU()
#     gnss = GNSS()
#     imu_index = 0
#     gnss_index = 0
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if gnss.time < imu_cur.time or not gnssfile.isEof():
            gnss = gnssfile.next()
            giengine.addGnssData(gnss)
    ...
    # np.savetxt(config['outputpath']+'/KF_GINS_IMU_ERR-py.txt', error_result, delimiter=" ",fmt="%6f")