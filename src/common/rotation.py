...
class RotationUtils:
    ...
    def quaternion2euler(quat):
        matrix = Rotation.from_quat(quat).as_matrix()
        return RotationUtils.matrix2euler(matrix)
    ...
    def quaternionleft(q):
        ...
        ans[1:4, 0]   = q[0:3]
        ans[1:4, 1:4] = q[3] * np.eye(3) + RotationUtils.skewSymmetric(q[0:3])
        return ans
    ...
    def quaternionright(p):
        ...
        ans[1:4, 0]   = p[0:3]
        ans[1:4, 1:4] = p[3] * np.eye(3) - RotationUtils.skewSymmetric(p[0:3])
        return ans
